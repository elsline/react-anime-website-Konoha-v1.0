...
function AnimeList() {
  ...
  useEffect(() => {
    ...
    dispatch(
      fetchAnimeList(`https://api.jikan.moe/v4/top/anime?q&page=${bulletID}`)
    ).then(() => {
      setIsLoading(false);
    });
  }, []);
  ...
  for (let i = 1; i <= bulNumber; i++) {
    bullets.push(
      <li
        ...
        key={i}
      >
        <Link onClick={() => handelDispatch(i)} to={`/animelist/top/${i}`}>
          {i}
        </Link>
      </li>
    );
  ...
  const handelDispatch = (i) => {
    ...
    setIsLoading(true);
    dispatch(
      fetchAnimeList(`https://api.jikan.moe/v4/top/anime?q&page=${i}`)
    ).then(() => {
      setIsLoading(false);
    });
  };
      ...
      <section className="main-bg-2">
        <div className="container  pt-5 search-page">
          <span className="fs-1 fw-medium text-uppercase text-center w-100 d-block mb-4 ">
            Top <span className="main-color">Anime</span>
          </span>
          <Row>
