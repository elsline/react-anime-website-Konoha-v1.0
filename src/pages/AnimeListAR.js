...
function AnimeListAR() {
  ...
  useEffect(() => {
    ...
    dispatch(
      fetchAnimeList(
        `https://api.jikan.moe/v4/top/anime?filter=airing&page=${bulletID}&sfw=true`
      )
    ).then(() => {
      setIsLoading(false);
    });
  ...
  for (let i = 1; i <= bulNumber; i++) {
    bullets.push(
      <li
        ...
        key={i}
      >
        <Link onClick={() => handelDispatch(i)} to={`/animelist/airing/${i}`}>
          {i}
        </Link>
      </li>
    );
  ...
  const handelDispatch = (i) => {
    ...
    dispatch(
      fetchAnimeList(
        `https://api.jikan.moe/v4/top/anime?filter=airing&page=${i}&sfw=true`
      )
    ).then(() => {
      setIsLoading(false);
    });
      ...
      <section className="main-bg-2">
        <div className="container  pt-5 search-page">
          <span className="fs-1 fw-medium text-uppercase text-center w-100 d-block mb-4 ">
            Most <span className="main-color">airing</span>
          </span>
          <Row>
